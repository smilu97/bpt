...
int height(MemoryPage * mem)
{
    ...
    while(false == c->header.is_leaf) {
        c = (InternalPage*)((get_page(table_id, (c->header.one_more_offset) / PAGE_SIZE))->p_page);
        ++h;
    }
    return h;
}
...
MemoryPage * find_left(MemoryPage * m_leaf)
{
    ...
    if(leaf_idx == 1) return get_page(table_id, parent->header.one_more_offset / PAGE_SIZE);
...
MemoryPage * find_right(MemoryPage * m_leaf)
{
    ...
    if(leaf_idx == parent->header.num_keys) return NULL;  // I'm most right!
...
int redistribute_leaf(MemoryPage * m_left, MemoryPage * m_right)
{
    ...
    if(len_left > len_right) {
        ...
        register_dirty_page(
            ...
            make_dirty(
                PAGE_HEADER_SIZE,
                PAGE_HEADER_SIZE + sizeof(Record) * right->header.num_keys
            )
        );
        ...
        register_dirty_page(
            ...
            make_dirty(
                ...
                PAGE_HEADER_SIZE + sizeof(Record) * left->header.num_keys
            )
        );
        register_dirty_page(m_right, make_dirty(0, PAGE_HEADER_COMMIT_SIZE));
        register_dirty_page(
...
int redistribute_internal(MemoryPage * m_left, MemoryPage * m_right)
{
    ...
    if(len_left > len_right) {
        ...
        register_dirty_page(
            ...
            make_dirty(
                PAGE_HEADER_SIZE - 8,
                PAGE_HEADER_SIZE + sizeof(InternalKeyValue) * right->header.num_keys
            )
        );
    } else if(len_left < len_right) {
        ...
        register_dirty_page(
            ...
            make_dirty(
                ...
                PAGE_HEADER_SIZE + sizeof(InternalKeyValue) * left->header.num_keys
            )
        );
        register_dirty_page(m_right, make_dirty(0, PAGE_HEADER_COMMIT_SIZE));
        register_dirty_page(
